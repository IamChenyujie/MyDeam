...
public class GoumaiDao {
    ...
    public GoumaiDao(Context context) {
        SQLiteGmDBOpenHelper helper = new SQLiteGmDBOpenHelper(context);
    ...
    public List<Gouwu> add(String name, String price, String store_name, int num, String url) {
        ...
        values.put("num", num);
        values.put("url",url);
        long result = db.insert("goumai", null, values);
    ...
    public List<Gouwu> finAll() {
        Cursor cursor = db.query("goumai", null, null, null, null, null, null);
//        Log.e("cursor",cursor.toString());
    ...
    public boolean update(String name, int num , String price){
        ...
        values.put("num", num);
        values.put("price", price);
        int result = db.update("goumai", values, "name = ?", new String[]{String.valueOf(name)});
        if(result > 0){
            return true;
        }else{
            return false;
    ...
    public boolean delete(String name){
         ...
         * 参数3：删除条件值  1
         */
        int result = db.delete("goumai", "name = ?", new String[]{String.valueOf(name)});
